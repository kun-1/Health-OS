...
function isDeductedThisMonth(rule: Rule): boolean {
  if (!rule.last_run_at) return false;
  return isSameMonth(new Date(rule.last_run_at), new Date());
}
...
export function RecurringManagerClient() {
  ...
  const remainingRules = activeRules.filter((r) => !isDeductedThisMonth(r));
  const remainingTotal = remainingRules.reduce((sum, r) => sum + r.amount_cents, 0);
  ...
  return (
    <div className="exp-analytics">
      ...
      {remainingRules.length > 0 ? (
        <div className="exp-recurring-hint">
          <span>本月剩余待扣固定支出：</span>
          <strong>{formatMoney(fromCents(remainingTotal), "CNY")}</strong>
          <span>（{remainingRules.length} 笔）</span>
        </div>
      ) : null}
      ...
      <section className="exp-col">
        ...
        {rules.length === 0 ? (
          ...
          <div className="exp-recurring-list">
            {rules.map((rule) => {
              const deducted = isDeductedThisMonth(rule);
              const days = daysUntil(rule.next_run_at);
              return (
                <div
                  className={`exp-recurring-row exp-job ${rule.active ? "" : "exp-recurring-row--inactive"} ${deducted ? "exp-recurring-row--deducted" : ""}`}
                  key={rule.id}
                >
                  <div className="exp-job__main">
                    <div className="exp-job__name">
                      <span aria-hidden className="exp-job__emoji">
                        {categoryEmoji(rule.category_zh)}
                      </span>
                      {rule.merchant_name} · {formatMoney(fromCents(rule.amount_cents), rule.currency)}
                    </div>
                    <div className="exp-card__meta">
                      {frequencyLabel(rule)} · {categoryLabel(rule.category_zh)}
                  ...
                  <div className="exp-job__status">
                    {deducted ? (
                      <span className="exp-job__status-tag exp-job__status-tag--done">本月已扣</span>
                    ) : (
                      <span className="exp-job__status-tag exp-job__status-tag--upcoming">
                        {days === 0 ? "今天扣款" : `还有 ${days} 天`}
                      </span>
                    )}
                  </div>
