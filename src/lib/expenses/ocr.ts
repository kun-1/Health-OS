...
6. amount 表示该商品行的金额。票据没有显示商品行金额时必须填 null，不要把重量、规格或数量写入 amount。
7. subtotal_amount 表示商品总额，total_amount 表示实际支付。
8. 中国生鲜订单如有配送费、配送费减免、活动优惠、加工费，要分别写入 delivery_fee、delivery_discount、discount_amount、processing_fee。
9. 商品明细尽量逐行提取；如果商品行无法可靠拆分，把无法确认原因写入 needs_review_reasons。票据缺少商品行金额时，只说明“商品行金额缺失”，不要编造金额。
10. confidence 使用 0 到 1 的数字，只表示图片文字识别和字段拆分的可靠性；不要因为票据本身缺少商家、时间或商品行金额而降低 confidence，这些缺失只写入 needs_review_reasons。
11. recognition_note 只写识别不确定性，不要写消费建议。
12. user_note 必须为 null。
13. model_suggested_auto_post 只有在商家、时间、总金额、商品明细、费用关系都清晰时才为 true。
14. 优先输出最终 JSON，推理保持简短，不要输出除 JSON 以外的内容。
